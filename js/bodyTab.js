...
layui.define(['layer',"element","jquery"],function(exports){
	...
	if($(".navBar").html() == ''){
		...
		var cometodown_loginuser = JSON.parse(sessionStorage.getItem("cometodown_loginuser"));
		$.post(cometodown_url+"/getPagesByUser",{loginId:cometodown_loginId,usertyperecid:cometodown_loginuser["utypeRecid"]},function(data){
			...
			if(data == "wdl"){
				...
				for(var i=0;i<data.length;i++){
					...
					if(data[i]["children"] != null && data[i]["children"] != ""){
						var level = data[i]["level"]*1+1;
						
						var childrens = [];
						var ids = [];
						if(data[i]["children"].indexOf(",")!=-1){
							ids = data[i]["children"].split(",");
						}else{
							ids.push(data[i]["children"]);
						}
						...
						$("#searchByindex").append('<option value="'+data[i].title+'">'+data[i].title+'</option>')
					}
					if(data[i]["level"] == 1){
						newdata.push(data[i]);
					}
				...
				var _this = this;
				var htmls = navBar(newdata);
				$(".navBar").html(htmls).height($(window).height()-230);
